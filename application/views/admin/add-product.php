	...
	<div class="pcoded-content">
		...
		<div class="row">
			...
			<div class="col-12">
				<div class="card">
					<div class="card-body" >
						<?=(!empty($product_id)) ? form_open_multipart('admin/update/products/add-product/' . $product_id[0]['id']) : form_open_multipart('admin/insert/products/products',array('autocomplete'=>'off'));   ?>
						<div class="row">
							<div class="col-sm-6">
							...
							<div class="col-sm-2">
								<div class="form-group">
									<label class="floating-label" for="tag">Tag</label>
									<select class="form-control" id="tag" name="product_tag">
										<option value="no_tag" <?=(!empty($product_id) AND $product_id[0]['product_tag'] === 'no_tag') ? "selected" : ""?>>No Tag</option>
										<option value="sale" <?=(!empty($product_id) AND $product_id[0]['product_tag'] === 'sale') ? "selected" : ""?>>Sale</option>
										<option value="featured" <?=(!empty($product_id) AND $product_id[0]['product_tag'] === 'featured') ? "selected" : ""?>>Featured</option>
										<option value="out_of_stock" <?=(!empty($product_id) AND $product_id[0]['product_tag'] === 'out_of_stock') ? "selected" : ""?>>Out of Stock</option>
									</select>
								</div>
							</div>
							<div class="col-sm-3">
							...
							<div class="col-sm-3">
								<div class="form-group">
									<label class="floating-label" for="Price">Discounted Price (optional)</label>
									<input type="number" name="d_price" step=".01" class="form-control" id="d_Price" value="<?=(!empty($product_id)) ? $product_id[0]['d_price'] : '' ?>" placeholder="in usd">
								</div>
							</div>
							<div class="col-sm-6">
								<div class="form-group">
...
<script>
	function delete_addOns(type,id,product_id,del_html){
		$.post("<?=site_url('admin/delete_product_addons/products/add-product')?>",{type,id,product_id},function (e){
	...
	function change_status(id){
		$.post("<?=site_url('super_admin/order_status')?>",{id},function(e){
			location.reload()
		})
	}
</script>
