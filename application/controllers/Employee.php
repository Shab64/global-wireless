...
class Employee extends CI_Controller
{
	...
	{
		$employee_login = $this->session->userdata('employee_login');
		if (!empty($employee_login)) {
			$id = $this->uri->segment(4);
			if (!empty($id)) {
				$data['product_id'] = $this->data->fetch('products', array('id' => $id));
			}
			$data['all_products'] = $this->data->fetch('products');
	...
	public function insert($table, $page)
	{
		...
		if ($table === 'products') {
			...
			$config['smtp_host'] = 'mail.devlabx.com';
			$config['smtp_user'] = 'shahab@devlabx.com';
			$config['smtp_pass'] = 'engineering644';
			$config['smtp_port'] = 465;
			$config['mailtype'] = "html";
			//                $this->email->initialize();
			$this->load->library('email', $config);
			$this->email->from('shahab@devlabx.com', 'Global Wireless');
                              ...
                              <div class="row">
                                ...
                                <div class="col-lg-9 email-content">
                                  <div class="email-body">
                                    <p>Hello ' . "<strong>" . $x['name'] . "</strong>" . ',</p>
	...
	public function do_login()
	{
		...
		$check = $this->data->fetch('user', $credentials);

		if (!empty($check)) {
			$this->session->set_userdata('employee_login', $check);
			redirect("employee");
		} else {
	...
	public function contact()
	{
		...
		$config['smtp_host'] = 'mail.devlabx.com';
		$config['smtp_user'] = 'abdul@devlabx.com';
		$config['smtp_pass'] = 'ali123ali123';
		$config['smtp_port'] = 465;
		$config['mailtype'] = "html";
		$this->email->initialize($config);
		$this->email->from('abdul@devlabx.com', 'Zenrolle');
		$this->email->to("mshahab644444@gmail.com");
		$this->email->set_newline("\r\n");
	...
	public function delete($table, $page, $id, $customer_id = null)
	{
		...
		if ($customer_id != null) {
			redirect('employee/view/' . $page . "/" . $id);
		} else {
			redirect('employee/view/' . $page);
		}
	}
	...
	public function edit($table)
	{
		...
		if ($table == 'user') { ?>
			<div class="modal fade" id="modal-edit" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
				<div class="modal-dialog modal-xl">
					<?= form_open_multipart('employee/update/user/employees/' . $data[0]['id']) ?>
		...
		if ($table == 'coupon') { ?>
			<div class="modal fade" id="modal-edit" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
				<div class="modal-dialog modal-xl">
					<?= form_open_multipart('employee/update/user/users/' . $data[0]['id']) ?>
					<div class="modal-content">
						<div class="modal-header">
			...
			<div class="modal fade" id="modal-report" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
				<div class="modal-dialog">
					<?= form_open_multipart('employee/update/products/products/' . $data[0]['id']) ?>
					<div class="modal-content">
						<div class="modal-header">
	...
	function updatePlanPrice()
	{
		...
		return redirect(base_url('employee/view/plan'));
	}
}
